...
`src/content/music/rotation.json` is generated by `npm run music:sync` and
drives section 03 of the homepage. There is no overrides file: the other logs
carry opinion worth correcting, and a hand-edited play count would be a
fabrication rather than a correction.

It is **one file, not one per track.** The other collections grow without
limit and are keyed by slug; this is a snapshot replaced wholesale each sync,
and the ranking *is* the content — per-track files would rewrite most of a
directory every day to record a reordering.

### Why this one needs a login

Play counts are not public. Not yours, not anyone's — and not even derivable
from your own public playlists, which is what makes a curated playlist a
different and less honest thing than this. `/me/top/tracks` is the only
endpoint that knows, and it answers only for a user who has said so in a
browser. So unlike the other four syncs, this one needs a **user grant**:

```sh
npm run music:auth    # once, ever. Opens Spotify, catches the redirect,
                      # prints a refresh token that does not expire.
npm run music:sync    # any time after that, unattended.
```

Three values, in `.env` locally and as repository secrets for the daily
workflow:

| Secret | From |
| --- | --- |
| `SPOTIFY_CLIENT_ID` | the app you create at developer.spotify.com/dashboard |
| `SPOTIFY_CLIENT_SECRET` | the same app's settings |
| `SPOTIFY_REFRESH_TOKEN` | `npm run music:auth`, once |
...
`music:auth` deliberately uses the classic authorization-code flow rather than
PKCE. PKCE would avoid needing the client secret, but Spotify rotates the
refresh token on every PKCE refresh — the stored secret would go stale and the
Actions run would need a new one each time. The classic flow issues one
long-lived token, which is what an unattended sync wants.

The scopes asked for are `user-top-read` and `user-read-recently-played`, and
nothing else. A token that only has to read a chart should not be able to
touch playlists or account details.

### What it can and cannot show

| Knob | Default | Other values |
| --- | --- | --- |
| `SPOTIFY_SOURCE` | `top` — most played | `recent` — last played, de-duplicated |
| `SPOTIFY_RANGE` | `short_term` ≈ 4 weeks | `medium_term` ≈ 6 months, `long_term` ≈ years |
| `SPOTIFY_LIMIT` | `12` | any count the column has room for |

Playback on the page is **Spotify's iframe embed** and cannot be anything
else. Their Web API stopped returning 30-second `preview_url`s to apps
registered after November 2024, so a custom player would have nothing to play,
and full tracks outside their player are not licensed in any case. The embed
gives a **signed-in listener the whole song and everyone else the preview** —
Spotify's rule, not a setting, and worth knowing before wondering why a track
stops at thirty seconds.

Clicking a row loads that track into the player, swaps the feature art, and
moves the meter. Every row is also a real link to the track, so with
