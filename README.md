...
npm run content:sync # all four of the above
...
`src/content/albums/*.json` is generated by `npm run albums:sync`, one file per
album with its photos inline. Corrections go in
`src/content/albums-overrides.json` — most usefully `description`, since Flickr
album descriptions are almost always empty.
...
`flickr.com/robots.txt` ends with `User-agent: *` / `Disallow: /`: a named
allowlist of about forty crawlers, and a closed door for everything else. So
the albums page is off limits, and the public API is the sanctioned way in —
better data anyway, with real sizes, real EXIF dates and real album structure
instead of parsed markup.

Only public photos are read, so an `api_key` is enough — no OAuth signing.
Get a non-commercial key (instant, free, and **it does not expire**, unlike
the PSN token) at
<https://www.flickr.com/services/apps/create/apply/>, then put
`FLICKR_API_KEY=...` in `.env` and in the **`FLICKR_API_KEY`** repository
secret.

Images are hotlinked from `live.staticflickr.com`, which serves them for
exactly this and whose robots.txt reads `# Nothing to see here`.

### The filmstrip is not a carousel

Each album on the index is a horizontally scrollable row, built as an
`overflow-x` container with `scroll-snap` and **no JavaScript at all**. The
browser's own scrolling handles trackpad, touch, shift-wheel and keyboard, so
none of it is reimplemented — and four photos are visible at once where a
carousel would show one and hide the rest behind a click.

Both the strip and the album page store each photo's real pixel dimensions and
set `width`/`height`, so every frame reserves its aspect ratio before the image
arrives and nothing reflows under a reader. The album page packs photos with
CSS `columns` rather than a grid, so portrait and landscape keep their shape
