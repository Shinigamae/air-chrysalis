...
**Step 4 — Real content.** The four logs are generated from their sources by
...
| `/workshop` | Case-study pattern + CLIENTS, 4 real clients |
| `/builds` + `/builds/<slug>` | Index with working status filter, 37 builds from Blogspot |
| `/games` + `/games/<slug>` | Currently-playing feature + index, 293 games from PSN |
| `/books` + `/books/<slug>` | Current book + reading timeline + index, 123 books from Goodreads |
| `/journeys` | 12 albums from Flickr — the route that used to be `/lab` |
...
**Three image slots still hold placeholders.** Everything else is real: the
four home cards, and the client art for BestMed, Menlo Security and Zespri's
mark. What is left is the Zespri screenshot and both of Soctrip's — and
Soctrip has no public site to shoot, so its plate may stay a placeholder for
good. Deleting the files is a valid end state there: `ClientCard` degrades to
the bare media well, though the `logo` / `shot` keys have to come out of
`src/content/clients/clients.json` with them.

A placeholder displays its own filename, dimensions and purpose, so replacing
one is a file swap with no code change. See `IMAGES.md` for the full mapping.
Build, game, book and album art is not local at all — it arrives hotlinked
with its feed.
...
~~`site.external.href` is a guess.~~ **Confirmed 2026-09-11** — the account
is real and holds this repo.

~~All content is seed data.~~ **Fixed 2026-09-11** — builds, games, books and
albums are generated by the syncs, and the Workshop's clients are the real
four. Nothing invented is left on the site.
...
3. **Finish the photography.** Three placeholders left, all in
   `src/assets/clients/` — the Zespri shot and both of Soctrip's. Overwrite a
...
5. ~~Decide what Lab is.~~ **Answered 2026-09-11** — it became `/journeys`,
   backed by the Flickr album collection.
6. ~~Deploy.~~ **Live** — see `DEPLOY.md`. Repo is public, Pages is on with
   Source: GitHub Actions, and `base: '/air-chrysalis'` is wired through.
   Every push to `main` ships.
7. ~~Interactive filters on Games and Books.~~ **Done 2026-09-09** for Games,
