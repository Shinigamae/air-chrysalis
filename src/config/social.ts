 ...
 * and the navigation, and burying six brand glyphs in it would have made
 * that file harder to read for no gain.
 *
 * Everything else on the homepage is an archive kept *here*. This section is
 * the opposite — six doors out — and the card built for it says so in its
...
export interface Profile {
   ...
   * the path is copied in rather than pulled from a package: six glyphs do
...
/**
 * Written in the order they should read — the row is three across on a wide
 * screen. LinkedIn leads because it is the door a client is looking for; the
 * rest carry no ranking.
 *
...
export const profiles: Profile[] = [
  {
    /*
     * Simple Icons dropped LinkedIn's mark in v11 at the brand's request, so
     * this path is the last one they published (v9), not a current export.
     * The handle is the profile slug, which is what LinkedIn shows in the URL.
     */
    key: 'linkedin',
    name: 'LINKEDIN',
    handle: 'in/ntkhanh',
  ...
  },
  {
    ...
    name: 'REDDIT',
    handle: 'u/shinigamae',
    href: 'https://www.reddit.com/user/shinigamae/',
